...
def correct_mot_hada_compound(text: str) -> tuple[str, list[str]]:
    ...
    **근거는 부정 근거다**(docs/log-archive/2026-h2.md §58 분류): 원문 형태(`부사+하다`)가 사전에 **없고**
...
def _specialist_reading_supported(target: str, context: str, surface: str) -> bool:
    ...
    ① **분야 일치**: `surface`(예: '건초')의 "전 용어" 뜻 그 자체에 분야가 달려
       ...
       묶은 그룹)로 넓혀 보지 않는다**(2026-09-02, docs/log-archive/2026-h2.md §94·BACKLOG 34번) — 예전엔
    ...
    **①은 양쪽 다 "그 뜻 하나만" 보도록 좁혔다**(2026-09-02 수정, docs/log-archive/2026-h2.md §90·§92·
    BACKLOG 34번). 처음엔 `target`(교체 목표어) 쪽만 `sense_fields(target)`
    (모든 뜻)에서 `former_term_field(surface)`(그 "전 용어" 뜻 하나)로 좁혀
    '정수'→'양수' 오탐지(docs/log-archive/2026-h2.md §90)를 고쳤는데, 검증하다 **문맥 낱말 쪽도 같은
    문제**를 새로 발견했다 — '감독'·'허가'처럼 일반 뜻과 무관한 전문 뜻(가톨릭
    성직 등)이 같이 있는 낱말이 그 무관한 뜻의 분야로 문맥 신호를 오염시켜
    '방안'→'모눈'·'소재'→'금육재'가 오탐지됐다(docs/log-archive/2026-h2.md §92). `sense_fields(lemma)`
        ...
        while p <= j:
            ...
            for q in range(j, p - 1, -1):
                ...
                if not result["ambiguous"]:
                    ...
                    # (`specialist_only_fields`, 2026-09-02, docs/log-archive/2026-h2.md §92·BACKLOG 34번), 그
