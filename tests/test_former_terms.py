...
class TestCorrectFormerTerms:
    ...
    def test_ambiguous_in_nondisease_context_is_not_flagged(self):
        """병명 문맥이 아니면 **묻지도 않는다**(2026-08-05 정책 변경, docs/log-archive/2026-h2.md §73).
...
class TestFormerTermContextEvidence:
    """동형이의 옛 용어를 **문맥 근거가 있을 때만** 묻는다(2026-08-05 사용자 보고, docs/log-archive/2026-h2.md §73).
    ...
    def test_generic_verb_overlap_does_not_support_specialist_reading(self):
        ...
        때문에 오탐지됐던 docs/log-archive/2026-h2.md §73 건초 사고와 같은 부류라 같은 불용어 목록에
        추가해 막았다."""
    ...
    def test_polysemous_context_word_does_not_support_specialist_reading(self):
        ...
        없는 낱말만 인정)로 바꿔 근본적으로 막았다(docs/log-archive/2026-h2.md §92) — '감독'·'허가'는
    ...
    def test_generic_noun_overlap_does_not_support_specialist_reading(self):
        ...
        때문에 오탐지됐던 docs/log-archive/2026-h2.md §73과 같은 부류라 같은 불용어 목록에 추가해 막았다."""
        flags = self._former_flags(
    ...
    def test_field_cluster_does_not_admit_unrelated_science_fields(self):
        ...
        기존 테스트는 하나도 없어서(docs/log-archive/2026-h2.md §93·§94 확인) 안전하게 지우고 정확히
        같은 분야만 근거로 인정하게 좁혔다(docs/log-archive/2026-h2.md §94)."""
        flags = self._former_flags(
