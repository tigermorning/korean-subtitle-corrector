...
**평가셋(`examples/eval/heldout.jsonl`, 41건)에 항목을 보탤 때는 `python tools/verify_eval_labels.py --file <쌍 목록>`으로 라벨을 먼저 검증하라.** gold가 틀리면 엔진이 아니라 눈금자가 휘는 것이라 더 위험하다. 이 도구는 "고칠 표기가 실제로 비표준이고, 고친 표기가 실제로 표준인가"를 쌍 단위로 판정하고, 방언·옛말 표제어(라벨 근거로 쓰면 안 되는 부류)를 따로 표시한다. 기준선은 **40/41 = 97.6%**(2026-08-03, §51).
