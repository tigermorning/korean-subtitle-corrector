...
**테스트 건수에 대해**: 시험 함수는 정적 집계로 440개다(매개변수화로 실행 시 항목 수는 더 많다). "444건 전부 통과"는 **2026-08-28 실측치**다(직전 401건은 2026-08-11 실측치였고 그 사이 다시 재지 않았었다) — `pytest`가 표준국어대사전·우리말샘 API를 실시간으로 조회하므로 네트워크와 API 키가 있어야 잴 수 있다. 재지 않은 숫자를 현재 상태로 적지 않는다.
