...
[`comparison_realusage.md`](comparison_realusage.md)에 있다.

### 무엇이 달라졌나 — 실제 문장

**AI 모델을 두 종 실제로 돌렸다. 그 실행이 "쓰지 말라"는 답을 냈다.**
...
### 이 PoC가 다루는 문제 — 실사용 사례 50건

[**`CASES.md`**](CASES.md)에 유형별로 정리했다. 전부 사용자가 실제 원고를 넣고
겪은 것이며 로그 절 번호가 붙어 있다.

| 유형 | 예 |
|---|---|
| A 표제어 있다고 붙임 | `나물 타령 하셨어` → `나물 타령하셨어` |
| B 반대로 갈라놓음 | `이런 말하지 마` → `이런 말 하지 마` |
| C 근거 없이 쪼갬 | `개나리길 입구에서 봐` → `입구에 서 봐` (**뜻이 바뀐다**) |
| D 명사 연쇄 오병합 | `남의 집 개는` → `남의 집개는` |
| E 조사·어미 오분리 | `말했잖아요` → `말했잖아 요` |
| F 규정 위반 제안 | `만성골수성백혈병` → `만성골수성 백혈병` (제50항 위반) |
| G 멀쩡한 말 플래그 | `부탁드릴 생각도 못 했는데` |
...
| 파일 | 내용 |
|---|---|
| [`CASES.md`](CASES.md) | **실사용 오교정 사례집 50건** — 이 PoC가 다루려는 문제. 전부 사용자가 실제로 겪고 로그에 기록된 것 |
