...
def run_b0(text: str, word: str | None = None) -> tuple[str, list[str]]:
    ...
    **답은 자동 교정된 본문이고, 플래그는 근거로만 센다.** 처음에는 플래그의
    ...
    "이렇게 고쳤다"로 잘못 기록된다(2026-08-12 실측: `술 시켜요`가 docs/log-archive/2026-h2.md §75에서 자동
...
# ---------------------------------------------------------------- B2a 낱말 겹침
#
# 저장소에 이미 있는 방식이다(docs/log-archive/2026-h2.md §73 `건초` 판정 — "표준 용어 뜻풀이와의 낱말 겹침").
...
# 이 arm은 그 뜻풀이를 꺼내 문맥과 대조하는 가드다. docs/log-archive/2026-h2.md §73의 `건초` 판정(표준 용어
